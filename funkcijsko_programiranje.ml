...
let zip_enum_tlrec l1 l2 =
  let rec zipper i acc l1 l2 =
  match (l1, l2) with
  | ([], []) -> rev acc
  | (x::xs, []) -> failwith "Seznama razlicnih dolzin."
  | ([], y::ys) -> failwith "Seznama razlicnih dolzin."
  | (x::xs, y::ys) -> zipper (i+1) ((i,x,y)::acc) xs ys
  in zipper 0 [] l1 l2
...
let unzip l =
  let rec unzipper l1 l2 l =
  match l with 
  | [] -> (rev l1, rev l2)
  | (a, b)::xs -> unzipper (a::l1) (b::l2) xs
  in unzipper [] [] l
...
let apply_sequence f x n =
  let rec aplajsek acc f x n =
    if n < 0 then [] 
	else 
	if n = 0 then rev acc 
	else 
	aplajsek ((f x)::acc) f (f x) (n-1) 
	in aplajsek [x] f x n
...
let filter f l =
  let rec filtering acc f l =
  match l with
  | [] -> rev acc
  | x::xs -> if (f x) = true then filtering (x::acc) f xs else filtering acc f xs
  in filtering [] f l
...
let exists f l = if filter f l = [] then false else true
...
let first f none_value l = if exists f l = true then List.hd (filter f l) else none_value
